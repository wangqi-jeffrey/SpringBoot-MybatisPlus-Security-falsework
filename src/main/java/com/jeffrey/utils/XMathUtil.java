...
public class XMathUtil {
     ...
     * 元转分
     */
    public static int yuan2fen(BigDecimal v) {
        if (null == v || v.compareTo(BigDecimal.ZERO) == 0) {
            return 0;
        }
        return multiply(v, ONE_HUNDRED).intValue();
    }

    /**
     * 分转元，保留两位小数
     */
    public static BigDecimal fen2yuan(Long v) {
        if (null == v || 0 == v) {
            return BigDecimal.valueOf(0, 2);
        }
        return BigDecimal.valueOf(v).divide(ONE_HUNDRED, DEF_DIV_SCALE, BigDecimal.ROUND_HALF_UP);
    }

    public static BigDecimal fen2yuan(Integer v) {
        if (null == v || 0 == v) {
            return BigDecimal.valueOf(0, 2);
        }
        return BigDecimal.valueOf(v).divide(ONE_HUNDRED, DEF_DIV_SCALE, BigDecimal.ROUND_HALF_UP);
    ...
    public static String fen2yuanStr(Integer v) {
        return fen2yuan(v).toString();
    }

    public static String fen2yuanStr(Long v) {
        return fen2yuan(v).toString();
    }

    public static BigDecimal fen2yuan(BigDecimal v) {
        if (null == v || v.compareTo(BigDecimal.ZERO) == 0) {
            return BigDecimal.valueOf(0, 2);
        }
        return v.divide(ONE_HUNDRED, DEF_DIV_SCALE, BigDecimal.ROUND_HALF_UP);
    }
